...
class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function getRandom(
        ...
        return $this->model->select($columns)->with($relations)->whereNotIn('id', $idNotIn)->where('status', 'active')->inRandomOrder()->limit($limit)->get();
    }
    /**
    ...
    /**
     * Update existing model or create a new one if not found.
     *
     * @param array $attributes   The attributes to update or create.
     * @param array $values       Additional values to set during creation.
     ...
     */
    // public function filter($filters, $with = [], $param = null): LengthAwarePaginator
    // {
    //     $query = $this->model->select('*')->filter($filters)->orderByColumn($filters['sortBy'], $filters['sortDirection']);
    //     if (count($with) > 0) {
    ...
    public function filterActive($filters, $with = []): LengthAwarePaginator
    {
        $query = $this->model->select('*')->where('status', 'active')->filter($filters)->orderByColumn($filters['sortBy'], $filters['sortDirection']);
        if (count($with) > 0) {
            $query = $query->with($with);
        }
        return $query->paginate($filters['rowPerPage'])->appends($filters);
